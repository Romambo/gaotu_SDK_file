...
2、 在TARGETS -> General -> Frameworks,Libraries,and Embedded Content栏目下添加以下framework：
   ...
   备注：并检查高图相关库的签名，只有AdjustSigSDK为动态库Embed & Sign ， 其他设置 Do not Embed  
   	AdServices.framework是iOS14.5新增库，需要设置可选，否则低版本会崩溃。在`Target` - `Build phases`选项卡中选择`Link Binary With Libraries`找到AdServices.framework,把它的Status由`Required`改为`Optional`  
