...
***3.4.4***
    ...
    1、新增用户防沉迷系统；
    ```
    [xxx gt_userLimitCallbackSwitchLogin:^{
        NSLog(@"需要游戏主动调用切换账号/登陆界面");
    } closeApp:^{
        NSLog(@"需要游戏主动关闭app：exit(0);");
    }];
    ```
    2、优化用户付费上报数据；
...
2、 在xcode中项目设置：TARGETS -> General -> Frameworks,Libraries,and Embedded Content栏目下添加以下系统库：
